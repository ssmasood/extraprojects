...
public:
    ...
    //Clears Buckets and set to null
    void clear();
...
private:
    ...
    unsigned int bCount;
    Node** buckets;
    void addTo(const std::string& key, const std::string& val, Node* &headPtr);
    void copy(Node** const &source, Node** target, unsigned int size);
    void setNull();
    void setNull(Node** &inBuckets, unsigned int size);
    void delBuckets();
    //delBuckets Helper
    void delNode(Node* source);
